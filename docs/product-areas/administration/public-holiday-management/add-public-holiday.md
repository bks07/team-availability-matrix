...
**IN ORDER TO** maintain the public holidays of a given location
**AS** admin user
**I WANT TO** add a public holiday for a given location

## ACCEPTANCE CRITERIA

1. An admin user can add a public holiday by providing a date, a name, and the location it applies to.
2. The date must be a valid calendar date.
3. The name must not be empty or consist only of whitespace.
4. A duplicate public holiday (same date and same location) is rejected with a clear error message.
5. After successful creation, the new public holiday appears in the list of public holidays for that location.
...
- Creating a new public holiday with a date, name, and associated location.
- Validation of inputs (valid date, non-empty name, no duplicate for that location and date).
...
- Locations already exist and can be selected when adding a public holiday.
...
1. Admin adds a public holiday with a valid date, name, and location — public holiday is created successfully.
2. Admin attempts to add a public holiday with a date and location that already exists — operation is rejected with an error message.
