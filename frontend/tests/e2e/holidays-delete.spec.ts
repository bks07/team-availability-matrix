...
test.describe("T3 - Delete public holiday", () => {
  ...
  test.beforeEach(async ({ request }) => {
    ...
    const holidayToDelete = await createHoliday(
      request,
      "2030-07-01",
      deleteHolidayName,
    );
  ...
  test("admin confirms deletion and holiday disappears", async ({ page }) => {
    ...
    await page
      .getByRole("button", { name: `Delete ${deleteHolidayName}` })
      .click();
    const dialog = await dialogPromise;

    expect(dialog.message()).toContain(
      `Delete public holiday "${deleteHolidayName}"?`,
    );
    await dialog.accept();

    await expect(
      page.getByText("Public holiday deleted successfully."),
    ).toBeVisible();
    await expect(
      page.getByRole("row", { name: new RegExp(deleteHolidayName) }),
    ).toHaveCount(0);

    await page
      .getByRole("button", { name: `Add location to ${cancelHolidayName}` })
      .click();
    const locationSelect = page.getByLabel("Select location to add");
    await expect(
      locationSelect.locator("option", { hasText: locationName }),
    ).toBeVisible();
  });
  ...
  test("admin cancels deletion and holiday remains", async ({ page }) => {
    ...
    const dialogPromise = page.waitForEvent("dialog");
    await page
      .getByRole("button", { name: `Delete ${cancelHolidayName}` })
      .click();
    const dialog = await dialogPromise;
    ...
    await expect(
      page.getByRole("row", { name: new RegExp(cancelHolidayName) }),
    ).toBeVisible();
  });
  ...
  test("non-admin cannot access admin holiday management", async ({ page }) => {
    ...
    await expect(
      page.getByText(
        "You do not have permission to view administration pages.",
      ),
    ).toBeVisible();
  });
});
