...
test.describe('T4 - Add location to holiday', () => {
  ...
  test('admin assigns location and sees it in holiday location list', async ({ page }) => {
    await adminLogin(page);
    await openHolidaysPage(page);

    await page.getByRole('button', { name: `Add location to ${holidayName}` }).click();
    await page.getByLabel('Select location to add').selectOption({ label: locationName });
    await page.getByRole('button', { name: 'Assign location' }).click();

    await expect(page.getByText(locationName)).toBeVisible();
  });

  test('admin sees duplicate error when assigning same location again', async ({ page, request }) => {
    await adminLogin(page);
    await openHolidaysPage(page);
