...
test.describe('T1 - Create public holiday', () => {
  ...
  test('admin creates holiday with date and name and sees it in the list', async ({ page }) => {
    ...
    await expect(page.getByText('Public holiday created successfully.')).toBeVisible();
    await expect(page.getByRole('row', { name: new RegExp(createdName) })).toBeVisible();
  });

  test('admin sees duplicate error for same date and name', async ({ page }) => {
    await adminLogin(page);
    await openHolidaysPage(page);
  ...
  test('admin sees validation error when holiday name is empty', async ({ page }) => {
    await adminLogin(page);
    await openHolidaysPage(page);
